...
parser$add_argument('--cell-clust', type='character', default="cell_id", help='Column name containing the cell IDs in the clustering file')
...
anno_top_k_genes <- function(chisqf, top_k) {
    ...
    return(df_chisq %>% 
        group_by(factor) %>% 
        slice_head(n = top_k) %>% 
        summarise(gene_string = paste0(sprintf("%02d", unique(factor)), "_", paste(gene, collapse = "_")), .groups = "drop") %>% 
        pull(gene_string))
}

load_lda_results <- function(tsvf, chisqf, offset_data, top_k, min_count) {
    ...
    df_tsv = df_tsv[, .SD, .SDcols = c(TRUE, mat_colsums>=min_count)]  # Select 1st and 3rd columns
    colnames(df_tsv)[1] = "cell_id"
...
lda_celltype_heatmaps = function(df_tsv, df_anno, celltype_name, offset_data) {
    ibeg = offset_data
    iend = ncol(df_tsv)
    df_merged = df_tsv %>% inner_join(df_anno, by = "cell_id")
...
df_tsv = load_lda_results(args$results, args$de_results, args$offset_data, args$top_k, args$min_count)

## load the clustering results
log_message("Loading clustering results")
df_clust = fread(args$clust)
#print(head(df_clust)) 
df_clust = df_clust %>% select(cell_id = !!args$cell_clust, cluster = !!args$colname_clust)
#print(head(df_clust)) 


## create heatmaps
log_message("Creating heatmaps")
heatmaps = lda_celltype_heatmaps(df_tsv, df_clust, args$colname_clust, args$offset_data)
