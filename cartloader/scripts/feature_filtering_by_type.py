...
def feature_filtering_by_type(_args):
    ...
    out_dir=os.path.dirname(args.out_feature) if args.out_feature is not None else os.getcwd()
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    
    logger = create_custom_logger(__name__, os.path.join(out_dir,__name__+".log") if args.log else None)
    ...
    ftrlist_keep_type= filter_feature_by_type(args.include_feature_type_regex, 
                                              ...
                                              logger=logger)
    log_info(f"  - N genes after applying --include-feature-type-regex : {len(ftrlist_keep_type)}", logger)
    ...
    if args.out_feature is not None:
