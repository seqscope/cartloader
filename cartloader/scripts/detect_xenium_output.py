...
def find_valid_path_from_zip(pattern, key, in_dir, unzip_dir, cellbounds, overwrite=False):
    for zip_fn in pattern["zips"]:
        ...
        zip_prefix, zip_suffix = split_prefix_suffix_from_compression(zip_in)
        
        assert os.path.exists(zip_in), f"An input compressed file does not exist: {zip_in}"

        # decompress the file 
        if not os.path.exists(unzip_dir) or overwrite:
        ...
        if os.path.exists(unzip_dir) and overwrite:
            print(f"Warning: the decompressed files will be overwritten: {unzip_dir}")    
...
def detect_xenium_output(_args):
    ...
    if all_items_defined(cellbounds):
        write_dict_to_file(cellbounds, args.out_json, check_equal=True)
    else:
        raise ValueError(f"Cannot find all source files from {args.in_dir}\nSee:\n{cellbounds}")
