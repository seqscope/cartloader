...
def parse_arguments(_args):
    ...
    aux_params.add_argument('--in-sge-assets', type=str, default="sge.json", help='The YAML/JSON file containing both the SGE files')
...
def run_xenium(_args):
    ...
    if args.fic_dir is not None:
        ...
        if len(in_fic_params) == 0: ## parameters are empty
            logging.error(f"The parameters are empty after loading {fic_jsonf}")
        ...
        for train_param in in_fic_params:
            ...
            model_rgb = str(Path(fic_dir / train_param["cmap"]).resolve())
    ...
    cmd = " ".join([
        ...
        ) + ([f"--cell-assets", args.cell] if args.cell else [])
    )
