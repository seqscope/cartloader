...
def parse_arguments(_args):
    ...
    key_params.add_argument('--georef-detect', type=str, default=None, help='If --georeference is required, detect the bounds from the image metadata. Supported formats: "ome" (read PhysicalSizeX/Y from the OME-XML), "gtiff" (the input already carries the intended geotransform but no CRS, e.g. an upstream-registered H&E TIF: keep its corner coordinates untouched and only assign --srs, which the tilers need to resolve a zoom range)')
    key_params.add_argument('--georef-plain', action='store_true', default=False, help='If --georeference is required, map the upper-left corner to (0,0) and the lower-right corner to the image size (in pixels), optionally scaled by --um-per-pixel. Use for images without embedded georeferencing metadata.')
    key_params.add_argument('--um-per-pixel', type=float, default=1.0, help='For --georef-plain, the number of microns (um) per pixel used to scale pixel coordinates into um units (default: 1.0)')
    ...
    key_params.add_argument('--method', type=str, default='geotiff2pmtiles', choices=['gdal','geotiff2pmtiles'], help='Method to convert image to pmtiles.')
