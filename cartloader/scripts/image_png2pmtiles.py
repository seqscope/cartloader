...
def parse_arguments(_args):
    ...
    """
    parser = argparse.ArgumentParser(prog=f"cartloader {inspect.getframeinfo(inspect.currentframe()).function}", description="Convert an image to pmtiles. Process: TIF/PNG to mbtiles, mbtiles to pmtiles.")
    ...
    cmd_params.add_argument('--geotif2mbtiles', action='store_true', default=False, help="")
    cmd_params.add_argument('--mbtiles2pmtiles', action='store_true', default=False, help="")
    ...
    inout_params.add_argument('--out-prefix', type=str, help='')
    ...
    key_params.add_argument('--georef-bounds-tsv', type=str, default=None, help='If --georeference is required, use the *.pixel.sorted.tsv.gz from run_ficture to provide georeferenced bounds.')
