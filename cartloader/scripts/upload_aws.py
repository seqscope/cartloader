...
def upload_aws(_args):
    ...
    multi_params = parser.add_argument_group("Optional Collection Upload Parameters", "Once set, enable a collection upload. This is designed as a shortcut to upload files generated by run_cartload2_multi function.")
    multi_params.add_argument('--in-list', type=str, default=None, help=('Path to input TSV without header. One sample per row with at least 1 columns [sample id]. This is the same file you used for run_ficture2_multi and run_cartload2_multi.'
                                                                        'If provided, --in-dir and --s3-dir should point to the parent directories containing all input and output subdirectories, respectively.'
                                                                        'Each input subdirectory should be named after the sample id in the first column of this file, and each output subdirectory will be named after the corresponding sample id in the second column of this file (if provided) or the first column if not.'))
