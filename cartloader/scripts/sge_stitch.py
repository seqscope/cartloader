...
def sge_stitch(_args):
    ...
    inout_params.add_argument("--in-tiles", type=str, nargs='*', default=[], help="List of the input SGEs tiles in a format of <transcript_path>,<feature_path>,<minmax_path>,<row>,<col>,<rotate>,<vertical_flip>,<horizontal_flip>. These are referred to as SGE tiles to distinguish them from the combined output SGE. ")
    inout_params.add_argument("--out-dir", type=str, help="Output directory for SGE format conversion results, density-filtered files, visualizations, and the Makefile")
    ...
    inout_params.add_argument('--out-tile-minmax', type=str, default="coordinate_minmax_per_tile.tsv", help='File name of the output coordinate minmax per tile TSV file to be written in --out-dir (default: coordinate_minmax_per_tile.tsv).')
    inout_params.add_argument('--out-json',  type=str, default=None, help='Path to the output json summarizing SGE information. (default: <out-dir>/sge_assets.json).')
    inout_params.add_argument('--feature-distribution', action='store_true', default=False, help='Enable generation of a feature distribution summary across input tiles (default: False)')
    inout_params.add_argument('--out-feature-distribution', type=str, default="feature.distribution.tsv.gz", help='File name of the output feature distribution summary across input SGE tiles to be written in --out-dir (default: feature.distribution.tsv.gz).')
    inout_params.add_argument('--filter-by-density', action='store_true', default=False, help='Enable density-filtering for the output SGE (default: False). If enabled, check the density-filtering auxiliary parameters.')
    inout_params.add_argument('--out-filtered-prefix', type=str, default="filtered", help='Name prefix for all density-filtered outputs, applied within --out-dir (default: filtered)')
    ...
    visual_params.add_argument('--out-northup-tif', type=str, default="xy_northup.tif", help='Name prefix for the output north-up image to be written in --out-dir (default: north_up.tif)')
