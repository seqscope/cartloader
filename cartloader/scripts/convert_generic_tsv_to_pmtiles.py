...
def convert_generic_tsv_to_pmtiles(_args):
    ...
    if not args.skip_pmtiles:
        ...
        cmd = f"{args.tippecanoe} -o {args.out_prefix}{args.out_pmtiles_suffix} -Z {args.min_zoom} -z {args.max_zoom} --force -s EPSG:3857 -M {args.max_tile_bytes} -O {args.max_feature_counts} --drop-densest-as-needed --extend-zooms-if-still-dropping '--preserve-point-density-threshold={args.preserve_point_density_thres}' --no-duplication --no-clipping --no-tile-size-limit --buffer 0 {args.out_prefix}{args.out_csv_suffix}"
