...
def parse_arguments(_args):
    ...
    inout_params.add_argument('--resolution', type=float, default=1.0, help='Resolution parameter for Leiden clustering (default: 1.0)')
    inout_params.add_argument('--use-weighting', action='store_true', default=False, help='Do not use weighting (default: False)')
...
def lda_leiden_cluster(_args):
    ...
    logger.info(f"Constructing k-NN graph (k={args.n_neighbors})...")
    adj_matrix = kneighbors_graph(
        mat_tsv, 
        n_neighbors=args.n_neighbors, 
        mode='distance' if args.use_weighting else 'connectivity', 
        metric='cosine',
        include_self=False,
        n_jobs=args.n_jobs
    )
    ...
    if args.use_weighting:
        weights = adj_matrix[sources, targets].A1      # cosine *distance* in [0, 2]
        weights = 1.0 - weights                        # convert to cosine *similarity* roughly in [−1, 1]
        g.es['weight'] = weights
    
    # Run Leiden
    logger.info("Running Leiden algorithm...")
    # RBConfigurationVertexPartition is standard for resolution parameter support
