...
### Load Xenium Ranger (`load_xenium_ranger`)

`load_xenium_ranger` scans a 10x Xenium Ranger output directory and writes a JSON manifest of detected assets. The manifest consolidates paths for SGE, cell analysis outputs (cells, boundaries, clusters, DE, embeddings), and morphology images (OME‑TIFFs).
