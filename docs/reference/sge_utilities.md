...
These utilities support specialized SGE preparation tasks that complement `sge_convert`. Use them to adjust orientation, or stitch SGE before analysis.
...
### Example Usage
Below showing an example with 90 rotation and horizontal flip.

```bash
OUT_PREFIX=r90_hflip                # Replace r90_hflip to your prefix
...
- `--out-transcript` (str): Output path for oriented transcript SGE TSV (gzipped).å
...
#### SGE Stitch Parameters
- `--out-dir` (str): Output directory for stitched SGE, filtered outputs, visualizations, and Makefile.
...
### Reorient SGE Output
Create an SGE matrix after orientation.

### SGE Stitching Output

Generate an SGE matrix consisting of all input SGE and in a specific layout. In addition:

* If `--filter-by-density` is enabled, a polygon-based filtered SGE matrix will be created from the stitched SGE.
